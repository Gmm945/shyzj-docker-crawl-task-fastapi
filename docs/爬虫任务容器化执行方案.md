...
## 远端 Docker 守护进程（DOCKER_HOST）与本地回调

当需要由本机 Worker 通过 Docker CLI 连接远端 Docker 守护进程时，可使用环境变量 `DOCKER_HOST` 指定 TCP 连接，例如：

```
DOCKER_HOST=tcp://172.28.214.138:2375
```

建议配套如下设置，确保容器内能回调到本机 Web：

- `.env` 中设置
  - `DOCKER_HOST=tcp://<远端IP>:2375`（2375 为非 TLS，仅供内网；生产建议 2376 TLS）
  - `DOCKER_HOST_IP=127.0.0.1`（使平台代码走“本地分支”，实际由 Docker CLI 通过 `DOCKER_HOST` 直连远端）
  - 可选：`API_BASE_URL=http://<宿主可被容器访问的地址>:8000`

- 远端 Docker 守护进程
  - 确保 dockerd 开启了相应 TCP 监听（2375/2376）。2375 请仅在可信网络使用；2376 需配置证书并在客户端设置 `DOCKER_TLS_VERIFY/DOCKER_CERT_PATH`。

- 本地回调可达性
  - 平台在本地运行时，会为容器注入 `--add-host host.docker.internal:host-gateway`，并默认将回调地址设为 `http://host.docker.internal:<API_PORT>`。
  - 如宿主为 Linux 且 `host-gateway` 不可用，请在 `.env` 显式设置 `API_BASE_URL=http://<宿主内网IP>:8000`。

- 容器保留与日志
  - 可通过 `.env` 控制容器保留：`DOCKER_AUTO_REMOVE=false` 与 `DOCKER_REMOVE_ON_STOP=false`，便于执行后查看日志：`docker logs <container_id>`。

这样即可在本机 Worker 与远端 Docker 执行环境之间，稳定完成拉起容器与心跳/完成回调的闭环。
